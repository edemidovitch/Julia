...
function data_res(db, symbol)
    ...
    r =
        DBInterface.execute(
            db,
            "select new_cases_smoothed_per_million from owid_covid_data where iso_code=? and date > '2020-03-10' and date <  date('now')",
            t,
        ) |> rowtable
    return r
end
...
symbols = ["USA", "CAN", "DEU", "GBR", "FRA", "ISR", "ESP","BEL","SWE", "DNK",  "CHE", "ITA", "UKR"]
